...
#define INT8VEC_MAXSIZE (SIZE_MAX - 100)
...
int i_int8vec_resize(int8vec* v, size_t tgtlen) {
  ...
  if (tgtcap < v->cap) {  // if overflow
    if (v->cap == INT8VEC_MAXSIZE) {
      return 0;
    }
    tgtcap = INT8VEC_MAXSIZE;
  }
...
static inline int i_int8vec_checkbound(int8vec* v, size_t len) {
  return len >= v->cap;
}
...
int int8vec_append(int8vec* v, int8_t c) {
  ...
  if (i_int8vec_checkbound(v, v->len + 1)) {
    return 2;
  }
  v->buf[v->len++] = c;
  return 0;
...
int int8vec_appendn(int8vec* v, int8_t* p, size_t n) {
  ...
  if (i_int8vec_checkbound(v, newlen)) {
    return 2;
  }
