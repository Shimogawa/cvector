...
#define INT64VEC_MAXSIZE (SIZE_MAX - 100)
...
int i_int64vec_resize(int64vec* v, size_t tgtlen) {
  ...
  if (tgtcap < v->cap) {  // if overflow
    if (v->cap == INT64VEC_MAXSIZE) {
      return 0;
    }
    tgtcap = INT64VEC_MAXSIZE;
  }
...
static inline int i_int64vec_checkbound(int64vec* v, size_t len) {
  return len >= v->cap;
}
...
int int64vec_append(int64vec* v, int64_t c) {
  ...
  if (i_int64vec_checkbound(v, v->len + 1)) {
    return 2;
  }
  v->buf[v->len++] = c;
  return 0;
...
int int64vec_appendn(int64vec* v, int64_t* p, size_t n) {
  ...
  if (i_int64vec_checkbound(v, newlen)) {
    return 2;
  }
